...
import {
	approveStakingLocked,
	configStakingLocked,
	stakeLocked,
} from '../../../../libs/stakingLocked';
...
export const LockedStakeProcess = ({
	...
	const getMinimalStakingTime = () => {
		let maxP = Math.ceil(
			configStakingLocked.maxEarningPass / configStakingLocked.valueEstimated
		);
	...
	const handleApprove = async () => {
		...
		try {
			...
			if (day >= lockDuration) {
				const res = await approveStakingLocked(
					tokenRequired.toString(),
					ethersSigner
				);
				setIsLoading(false);
	...
	const estimatedReceivableRewards =
		lockDuration > 2
			? Math.floor(blockNum * lockDuration * configStakingLocked.valueEstimated)
			: 0;
	return (
		<Box>
			...
			<Item sx={{ display: 'flex', mt: '16px !important' }}>
				...
				<Box sx={{ marginLeft: '8px' }}>
					<ButtonCustom
						variant="text"
						disabled={lockDuration === 30 || status === 1}
						onClick={handleClickIncrementDay}
					>
						<ArrowUp />
					</ButtonCustom>
					<ButtonCustom
						variant="text"
						disabled={lockDuration < 4 || status === 1}
						sx={{ mt: '2px' }}
						onClick={handleClickDecrementDay}
					>
						<ArrowDown />
					</ButtonCustom>
				</Box>
			...
			{lockDurationError === true && (
				<Item sx={{ mt: '8px !important' }}>
					...
					>
						You have reached maximum reward!
						<br />
						Suggest: With {lockDurationSuggest.toString()} days of lock
						duration, you still maximize your rewards without locking your token
						for too long.
					</TitleItem>
				</Item>
			)}
