...
export const WithDraw = (props: Props) => {
	...
	const timeUTC = () => {
		let time = new Date(new Date().getTime() + parseFloat(remainingDelayTime));
	...
	return (
		<>
			<Box sx={{ display: "flex", justifyContent: "center", flexDirection: "column", alignItems: "center", height: "100%" }}>
				...
				<Typography fontSize={14} color="#5A6178" textAlign={"center"} fontWeight={500} mt="24px" textTransform={"uppercase"}>avaiLable amount</Typography>
				<Typography fontSize={14} color="#31373E" textAlign={"center"} fontWeight={600} mt="8px" textTransform={"uppercase"}>{balanceUS} FIU</Typography>
			</Box>
			{
			...
			<Box width={"100%"} sx={{ paddingTop: "16px", borderTop: "1px solid #E9EAEF", mt: "16px" }}>
				<MarketplaceButton disabled={parseFloat(balanceUS) <= 0} customStyle={{ width: "100%" }} title={"Withdraw"} handleOnClick={handleWithdraw} />
			</Box>
...
export const WithDrawWarning = (props: Props) => {
	...
	const { ethersProvider, ethersSigner, setRefresh, refresh } = useWalletContext();
		...
		<>
			...
			<Box sx={{ display: "flex", justifyContent: "center", flexDirection: "column", alignItems: "center", height: "100%" }}>
				<Box sx={{ textAlign: "center", }}>
					...
					<Typography fontSize={14} color="#31373E" fontWeight={500} lineHeight="20px" mt="28px">If you want to withdraw immediately, you have to pay a penalty that will <span style={{ color: "#FF6F61" }}>cost 16% </span> of your staked amount to skip the delay time. Are you sure want to continue?</Typography>
				</Box>
			</Box>
			<Box mt="auto" width={"100%"} sx={{ paddingTop: "16px", borderTop: "1px solid #E9EAEF" }}>
				<MarketplaceButton customStyle={{ width: "100%" }} title={"Continue"} handleOnClick={handleWithdraw} />
				<ButtonOutline onClick={handleCancel} sx={{ width: "100%", marginTop: "8px", color: "#31373E", height: "38px", flex: 1, borderColor: "#E9EAEF" }} variant="text">
					Cancel
				</ButtonOutline>
			</Box>
