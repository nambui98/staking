  ...
  return (
    <Wrap>
      ...
      <BoxItem>
        <Box sx={{
          ...
          <Box>
            {BUSINESS_MODEL.body1?.map((item, index) => (
              <Item sx={item1} key={index}>{item}</Item>
            ))}
          </Box>
          {!isMobile && <Item sx={itemImage1}><img src={BUSINESS_IMAGE.shoeDouble} /></Item>}
        </Box>
        <Item sx={{
          margin: '24px 0',
        ...
        }}>{BUSINESS_MODEL.body2}</Item>
        {isMobile && <Item sx={{...itemImage1, marginBottom: '40px'}}><img src={BUSINESS_IMAGE.shoeDouble} /></Item>}
        <Box sx={{
          display: 'flex',
          alignItems: 'center',
          ...
          {!isMobile && <Item sx={itemImage2}><img src={BUSINESS_IMAGE.logoOrange} /></Item>}
          <Box>
            {BUSINESS_MODEL.body3?.map((item, index) => (
              <Item key={index} sx={item2}>{item} {index === 1 && isMobile && <img style={{width: '138px'}} src={BUSINESS_IMAGE.logoOrange} />}</Item>
            ))}
          </Box>
        </Box>
      </BoxItem>
