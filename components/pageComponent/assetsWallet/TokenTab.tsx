...
export const TokenTab: React.FC<IProps> = ({ tokenChoose, setTokenChoose }) => {
	...
	return (
		<Wrap>
			<Item
				tokenName={tokenChoose === 'fiu' ? 'fiu' : ''}
				sx={ItemFiu}
				onClick={() => setTokenChoose('fiu')}
			>
				<ItemLeft>
					<Title>Your balance</Title>
					<Amount typeBnb={false}>
						{fiuBalance?.length && parseFloat(fiuBalance) > 0
							? formatMoney(fiuBalance)
							: '0.00'}
						<span>FIU</span>
					</Amount>
				</ItemLeft>
			...
			<Item
				tokenName={tokenChoose === 'hee' ? 'hee' : ''}
				sx={ItemHee}
				onClick={() => setTokenChoose('hee')}
			>
				<ItemLeft>
					<Title>Your balance</Title>
					<Amount typeBnb={false}>
						{heeBalance?.length && parseFloat(heeBalance) > 0
							? formatMoney(heeBalance)
							: '0.00'}
						<span>HEE</span>
					</Amount>
				</ItemLeft>
			...
			<Item
				tokenName={tokenChoose === 'bnb' ? 'bnb' : ''}
				sx={ItemBnb}
				onClick={() => setTokenChoose('bnb')}
			>
				<ItemLeft>
...
const ItemFiu = {
	background:
		'linear-gradient(84.49deg, #FF612F 2.08%, rgba(255, 109, 36, 0) 70.33%)',
};
const ItemHee = {
	background:
		'linear-gradient(84.49deg, #1DB268 2.08%, rgba(29, 178, 104, 0) 67.71%)',
};
const ItemBnb = {
	background:
		'linear-gradient(84.49deg, #FFC83A 2.08%, rgba(255, 200, 58, 0) 67.19%)',
};
